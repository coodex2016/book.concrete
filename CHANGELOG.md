...
- concrete-core: 增加TokenBaseTopic的订阅管理切片，和自动取消订阅的TokenEventListener，[使用参考](https://github.com/coodex2016/concrete.coodex.org/blob/0.2.x/test-case/test-case-server/src/main/java/org/coodex/concrete/test/impl/SubscribeInterceptor.java)
...
- 增加对rxjava2的支持
    ...
    - [X] 增加concrete-jaxrs-client-rx，将concrete-jaxrs-client转成rx，参见[JavaClient](impl/JavaClient.md)
...
- api: [log](definition/log.md)注解进行调整
- 添加[账户模块](accounts/README.md)，提供组织结构类型的账户管理框架及参考实现
...
- coodex-utilities: POJOMocker推倒从来，原接口保留到0.2.1，0.2.2版将移除。新[Mocker看这](impl/mocker.md)。
...
- 增加请求附加信息[Subjoin](definition/Subjoin.md)，在jaxrs模块中利用Header实现
- api/core部分少量接口、工具调整package
- 增加可签名[Signable](definition/Signable.md)注解，用于定义模块或者单元是否需要签名
...
- coodex-utilities中定义了一个[计数模型](impl/counter.md)，目的：数据随时产生随时统计或统计预处理，不需要所有的统计都到数据库里去算，分段统计快照，提高统计效率
...
- 重构javaClient，调整机制，简化client端构建
    ...
    - 支持字符集设定，参见[Java客户端调用](impl/JavaClient.md)
...
- 提供最大业务并发量模型的默认实现，后续扩展令牌桶、漏桶模型的默认实现
    ...
    ```properties
      ...
      strategyName.max
    ```
- [api工具](impl/API.md)增加angular支持，支持angular2+
...
[@Description](definition/Description.md) 增强装饰功能。
...
增加[附件管理模块](impl/fileServer.md)及基于Jax-RS 2.0 和 [FastDFS](https://github.com/happyfish100)的参考实现。
...
* 工具链增加了[java客户端](impl/JavaClient.md)
